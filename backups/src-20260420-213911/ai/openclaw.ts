...
export class OpenClawClient {
	...
	private async authenticate(): Promise<void> {
		...
		return new Promise((resolve, reject) => {
			...
			const messageHandler = (event: MessageEvent) => {
				...
				// Wait for hello-ok
				const helloHandler = (event: MessageEvent) => {
					const msg = JSON.parse(event.data);
					if (msg.type !== 'res' || msg.payload?.type !== 'hello-ok') return;
	...
	async createSession(agentId: string = 'diaryagent'): Promise<string> {
	...
	async analyzeBlock(blockContent: string, blockId: string): Promise<AnalysisResult> {
...
请分析并回复 JSON 格式（不带 markdown 代码块）：
{
  ...
  "aiResponse": "对用户的友好回复，不超过100字"
	...
	private parseAnalysisResponse(blockId: string, content: string, response: string): AnalysisResult {
		try {
			...
			if (jsonMatch) {
				...
				return {
					...
					entities: {
						people: (parsed.entities?.people || []).map((p: any) => ({
						...
						projects: (parsed.entities?.projects || []).map((p: any) => ({
						...
						things: (parsed.entities?.things || []).map((t: any) => ({
							type: 'thing' as const,
							name: t.name,
						...
						ideas: (parsed.entities?.ideas || []).map((i: any) => ({
							type: 'idea' as const,
							name: i.name,
						...
						knowledge: (parsed.entities?.knowledge || []).map((k: any) => ({
